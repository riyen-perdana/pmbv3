<template>
                    ...
                    <div class="row mb-3 pb-1 p-3">
                        <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                            <div class="flex-grow-1">
                                <Titlebox :isDashboard="false" title="Data Agama"
                                    subTitle="Manajemen Data Agama Aplikasi E-Admisi" />
                            </div>
                                        ...
                                        <div class="col-md-4">
                                            <div class="search-box">
                                                <input type="text" class="form-control search bg-light"
                                                    placeholder="Cari Data Agama" v-model="data.params.search">
                                    ...
                                    <div class="row">
                                        <div>
                                            <div class="table-responsive table-card mb-3 mt-3">
                                                <table class="table table-hover align-middle table-nowrap mb-0"
                                                    ...
                                                    <thead class="table-light boder-white">
                                                        <tr>
                                                            ...
                                                            <th scope="col" style="width:80%;">Agama</th>
                                                            <th scope="col" style="width: 13%;">Aksi</th>
                                                        </tr>
                                                    </thead>
                                                    <tbody>
                                                        <tr v-for="(agm, index) in agama.data" :key="index">
                                                            <td class="r_atas">{{ (agama.current_page - 1) * data.params.perPage + index + 1 }}</td>
                                                            <td class="r-atas">{{ agm.nm_agama }}</td>
                                                            <td class="r_atas">
                                                                <div class="hstack gap-2 flex-wrap">
                                                                    <!-- Buttons Group -->
                                                                    <button type="button"
                                                                        class="btn btn-secondary btn-sm"
                                                                        @click="clickButton(agm.id, agm)"><i
                                                                            class="ri-edit-2-line label-icon align-middle fs-10 me-1"></i>
                                                                        Edit&nbsp;&nbsp;&nbsp;</button>
                                                                    <button type="button" class="btn btn-danger btn-sm"
                                                                        @click="destroy(agm.id)"><i
                                                ...
                                                <Emptytable :data="props.agama" />
                                            </div>
                                            <div v-if="agama.data.length != 0">
                                                <Pagination :links="agama.links" :count="agama" :perPage="props.perPage" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
    ...
    <Alert :show="data.openAlert" :id="data.id" @close-alert="closeAlert" @delete-data="deleteAgama" />
</template>
...
import Form from "@/Pages/Apps/Agama/Form.vue";
    ...
    router.delete(`/apps/agama/${id}`,
    {
        ...
        onSuccess : () => {
            ...
            createToast(
                {
                    title: 'Berhasil',
                    description: 'Data Agama Berhasil Dihapus.'
                }, {
                    type: 'success',
                    showIcon: true,
                    transition: 'zoom',
        ...
        onError : () => {
            ...
            createToast(
                {
                    title: 'Error',
                    description: 'Gagal Menghapus Data Agama.'
                }, {
                    type: 'danger',
                    showIcon: 'true',
                    transition: 'zoom',
