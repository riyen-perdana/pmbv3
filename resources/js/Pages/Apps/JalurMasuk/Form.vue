<template>
  <Modal id="mJalurMasuk" size="modal-lg" @close="closeModal">
    <template #modalTitle
      >{{ props.isEdit == false ? "Tambah Data" : "Ubah Data" }} Jenjang
      Pendidikan</template
    >
    <template #modalBody>
      <form @submit.prevent="submitData()">
        <div class="row">
          <div class="col-md-6 mb-3">
            <label for="nmJalurMasuk" class="form-label"
              >Nama Jalur Masuk</label
            >
            <input
              id="nmJalurMasuk"
              v-model="form.nmJalurMasuk"
              class="form-control"
              :class="{ 'is-invalid': $page.props.errors.nmJalurMasuk }"
              type="text"
              placeholder="Isikan Jalur Masuk"
            />
            <div
              v-if="$page.props.errors.nmJalurMasuk"
          ...
          <div class="col-md-12 lg-12 mb-3">
            <label for="preJalurMasuk" class="form-label"
              >Prefiks Jalur Masuk</label
            >
            <input
              v-model="form.preJalurMasuk"
              class="form-control"
              :class="{ 'is-invalid': $page.props.errors.preJalurMasuk }"
              type="text"
              placeholder="Isikan Prefiks Jalur Masuk"
            />
            <div
              v-if="$page.props.errors.preJalurMasuk"
              class="invalid-feedback"
            >
    ...
    <template #modalFooter>
      ...
      >
        {{ props.isEdit == false ? "Simpan" : "Ubah" }}
      </button>
    </template>
  </Modal>
</template>
...
const submitData = () => {
  if (props.isEdit) {
    router.put(
      `/apps/jenjang/${props.dataEdit.id}`,
      {
            ...
            createToast(
              {
                title: "Berhasil",
                description:
                  "Perubahan Data Jenjang Pendidikan Berhasil Dilakukan.",
              },
              {
                type: "success",
                showIcon: true,
            ...
            createToast(
              {
                title: "Berhasil",
                description:
                  "Penambahan Data Jenjang Pendidikan Berhasil Dilakukan.",
              },
              {
                type: "success",
                showIcon: true,
