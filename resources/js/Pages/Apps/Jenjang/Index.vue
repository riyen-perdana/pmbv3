<template>
    ...
    <div class="container-fluid">
                    ...
                    <div class="row mb-3 pb-1 p-3">
                        <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                            <div class="flex-grow-1">
                                <Titlebox :isDashboard="false" title="Data Jenjang Pendidikan"
                                        ...
                                        <div class="col-md-4">
                                            <div class="search-box">
                                                <input type="text" class="form-control search bg-light"
                                                    placeholder="Cari Data Jenjang Pendidikan" v-model="data.params.search">
                                    ...
                                    <div class="row">
                                        <div>
                                            <div class="table-responsive table-card mb-3 mt-3">
                                                <table class="table table-hover align-middle table-nowrap mb-0" id="jenjangTable">
                                                    <thead class="table-light boder-white">
                                                        <tr>
                                                            <th scope="col" style="width:4%;">No.</th>
                                                            <th scope="col" style="width:10%;">Kode Jenjang</th>
                                                            <th scope="col" style="width:63%;">Jenjang Pendidikan</th>
                                                            <th scope="col" style="width:10%;">Status Aktif</th>
                                                            <th scope="col" style="width: 13%;">Aksi</th>
                                                        </tr>
                                                    </thead>
                                                    <tbody>
                                                        <tr v-for="(jjng, index) in jenjang.data" :key="index">
                                                            <td class="r_atas">{{ (jenjang.current_page - 1) * jenjang.per_page + index + 1 }}.</td>
                                                            <td class="r_atas">{{ jjng.kd_jenjang }}</td>
                                                            <td class="r_atas">{{ jjng.nm_jenjang }}</td>
                                                            <td class="r_atas"><span :class="jjng.is_aktif == 'Y' ? 'badge bg-success' : 'badge bg-danger'">{{ jjng.is_aktif == 'Y' ? 'Aktif' : 'Tidak Aktif'}}</span></td>
                                                            <td class="r_atas">
                                                                <div class="hstack gap-2 flex-wrap">
                                                                    <!-- Buttons Group -->
                                                                    <button type="button" class="btn btn-secondary btn-sm"
                                                                        @click="clickButton(jjng.id, jjng)"><i
                                                                            class="ri-edit-2-line label-icon align-middle fs-10 me-1"></i>
                                                                        Edit&nbsp;&nbsp;&nbsp;</button>
                                                                    <button type="button" class="btn btn-danger btn-sm"
                                                                        @click="destroy(jjng.id)"><i
        ...
        <Alert :show="data.openAlert" :id="data.id" @close-alert="closeAlert" @delete-data="deleteJenjang" />
    </div>
</template>
