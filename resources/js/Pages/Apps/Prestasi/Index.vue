<template>
                    ...
                    <div class="row mb-3 pb-1 p-3">
                        <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                            <div class="flex-grow-1">
                                <Titlebox :isDashboard="false" title="Data Bidang Prestasi"
                                    subTitle="Manajemen Data Bidang Prestasi Aplikasi E-Admisi" />
                            </div>
                                        ...
                                        <div class="col-md-4">
                                            <div class="search-box">
                                                <input type="text" class="form-control search bg-light"
                                                    placeholder="Cari Data Bidang Prestasi" v-model="data.params.search">
                                    ...
                                    <div class="row">
                                        <div>
                                            <div class="table-responsive table-card mb-3 mt-3">
                                                <table class="table table-hover align-middle table-nowrap mb-0"
                                                    ...
                                                    <thead class="table-light boder-white">
                                                        <tr>
                                                            ...
                                                            <th scope="col" style="width:20%;">Nama Bidang Prestasi</th>
                                                            <th scope="col" style="width:60%;">Nilai</th>
                                                            <th scope="col" style="width: 13%;">Aksi</th>
                                                        </tr>
                                                    </thead>
                                                    <tbody>
                                                        <tr v-for="(bdg, index) in bidang.data" :key="index">
                                                            <td class="r_atas">{{ (bidang.current_page - 1) * data.params.perPage + index + 1 }}</td>
                                                            <td class="r-atas">{{ bdg.nama }}</td>
                                                            <td class="r_atas">{{ bdg.nilai }}</td>
                                                            <td class="r_atas">
                                                                <div class="hstack gap-2 flex-wrap">
                                                                    <!-- Buttons Group -->
                                                                    <button type="button"
                                                                        class="btn btn-secondary btn-sm"
                                                                        @click="clickButton(bdg.id, bdg)"><i
                                                                            class="ri-edit-2-line label-icon align-middle fs-10 me-1"></i>
                                                                        Edit&nbsp;&nbsp;&nbsp;</button>
                                                                    <button type="button" class="btn btn-danger btn-sm"
                                                                        @click="destroy(bdg.id)"><i
                                                ...
                                                <Emptytable :data="props.bidang" />
                                            </div>
                                            <div v-if="bidang.data.length != 0">
                                                <Pagination :links="bidang.links" :count="bidang" :perPage="props.perPage" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
    ...
    <Alert :show="data.openAlert" :id="data.id" @close-alert="closeAlert" @delete-data="deletePrestasi" />
</template>
...
import Form from "@/Pages/Apps/Prestasi/Form.vue";
...
const clickButton = (id, bidang) => {
    data.id = id
    if (id == '') {
        data.isEdit = false
    } else {
        data.isEdit = true
        data.dataEdit = bidang;
    }
...
const deletePrestasi = (id) => {
    router.delete(`/apps/prestasi/${id}`,
    {
        preserveState: true,
        preserveScroll: true,
        onSuccess : () => {
            ...
            createToast(
                {
                    title: 'Berhasil',
                    description: 'Data Bidang Prestasi Berhasil Dihapus.'
                }, {
                    type: 'success',
                    showIcon: true,
                    transition: 'zoom',
        ...
        onError : () => {
            ...
            createToast(
                {
                    title: 'Error',
                    description: 'Gagal Menghapus Data Bidang Prestasi.'
                }, {
                    type: 'danger',
                    showIcon: 'true',
                    transition: 'zoom',
...
watch(() => cloneDeep(data.params), debounce(() => {
    ...
    router.get('/apps/prestasi', param, {
        replace: true,
        preserveState: true,
        preserveScroll: true
    })
